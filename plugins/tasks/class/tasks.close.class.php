...
var $lang = array();
	...
	function __construct($controller) {
		...
		$this->settings   = PROFILESDIR.'/tasks.ini';
	...
	function action($action = null) {
		...
		if($response instanceof htmlobject_response) {
			...
			if(!isset($response->msg)) {
				...
				$t = $response->html->template($this->tpldir.'tasks.close.html');
				$t->add($vars);
				$t->add($response->form);
	...
	function close() {
		...
		if( $elements !== '' ) {
			...
			foreach($elements as $id) {
				$d['param_f'.$i]['label']                       = sprintf($this->lang['label_tasks'], $id);
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($elements as $key => $id) {
					$error = $this->db->update( 'tasks_tasks', array('updater' => 'closed', 'updated' => time()), array('id', $this->id) );
					if($error === '') {
						$user = $this->user->get();
