...
var $lang = array();
	...
	function action($action = null) {
		...
		case 'insert':
			...
			$t = $this->response->html->template($this->tpldir.'tasks.notice.insert.html');
		...
		case 'delete':
			...
			elseif(isset($response->error)){
				$this->response->redirect(
					$this->response->get_url(
						$this->controller->actions_name, 'update', $this->message_param.'[error]', $response->error
					)
				);
			}
		break;
	...
	function read() {
		...
		$notices = '';
		$user    = $this->user->get();
		$result  = $this->db->select('tasks_notices', '*', array('task', $this->id));
		if(is_array($result)) {
			rsort($result);
			foreach($result as $k => $v) {
				...
				if($display === true) {
					...
					if(isset($v['login']) && $v['login'] === $user['login']) {
						...
						$del->label = '';
						$del->href  = $this->response->get_url($this->actions_name, 'delete').'&notice='.$v['id'].'&task='.$this->id;
						$del->title = $this->lang['notice_delete'];
						$del->css   = 'delete icon icon-trash';
						$del->handler = 'onclick="Tasks.confirm(this, \''.$v['id'].'\'); return false;"';
					}

					// handle mkpublic
					$pub = '&#160;';
					if($this->user->is_admin() === true || (isset($v['login']) && $v['login'] === $user['login'])) {
					...
					*/
					...
					if(isset($this->controller->settings['settings']['attachment'])) {
						$result = $this->db->select('tasks_attachments', array('file', 'name'), array('task' => $this->id, 'notice' => $v['id']));
						if(is_array($result)) {
					...
					$n = $this->response->html->template($this->tpldir.'tasks.notice.read.html');
	...
	function insert() {
		...
		if(!$form->get_errors() && $response->submit()) {
			if(!isset($response->error)) {
				$private = $form->get_request('private', true);
				$login   = '';
				$private = (isset($private) && $private !== '') ? 1 : '';
				$user    = $this->user->get();
				if(isset($user)) {
					$login = $user['login'];
				}
				...
				if(
					...
					$path = $this->controller->profilesdir.'/tasks/attachments/'.$this->id;
	...
	function mkpublic() {
		...
		if($notice !== '') {
			$result = $this->db->select('tasks_notices', array('private','login'), array('id', $notice));
			if(is_array($result)) {
				if($user['login'] === $result[0]['login']) {
					$private = (isset($result[0]['private'])) ? '' : 1;
					$error = $this->db->update('tasks_notices', array('private' => $private), array('id', $notice));
					if($error !== '') {
						$response->error = $error;
					} else {
						$response->msg = '';
	...
	function delete() {
		$response = $this->response;
		$notice   = $response->html->request()->get('notice');
		$tasks   = $response->html->request()->get('task');
		if($tasks !== '' && $notice !== '') {
		
			// check owner
			$user  = $this->user->get();
			$check = $this->db->select('tasks_notices','login', array('id', $notice));
			if(
				is_array($check) && 
				isset($check[0]['login']) && 
				$user['login'] === $check[0]['login']
			) {
				// delete attachment
				$files = $this->db->select('tasks_attachments','row,file', array('notice', $notice));
				if(is_array($files)) {
					$row  = $files[0]['row'];
					$path = $this->controller->profilesdir.'tasks/attachments/'.$tasks.'/'.$files[0]['file'];
					if($this->file->exists($path)) {
						$error = $this->file->remove($path);
						if($error === '') {
							$error = $this->db->delete('tasks_attachments', array('row', $row));
						}
					}
				}
				if(!isset($error) || $error === '') {
					$error = $this->db->delete('tasks_notices', array('id', $notice));
				}
				if(isset($error) && $error !== '') {
					$response->error = $error;
				} else {
				...
				elseif(isset($check[0]['login']) && $user['login'] !== $check[0]['login']) {
					$response->error = 'Permission denied';
				}
				else {
					$response->msg = '';
	...
	function get_response() {
		...
		$columns = $this->db->handler()->columns($this->db->db, 'tasks_notices');
