...
var $substr = 200;
	...
	function action() {
		...
		$t = $response->html->template($this->tpldir.'tasks.select.html');
		$t->add($vars);
		$t->add($response->form);
	...
	function select() {
		...
		$head['tasks']['title'] = $this->lang['select_task'];
		$head['tasks']['sortable'] = false;
				...
				if(isset($f['updated']) && $f['updated'] !== '') {
					...
					if($f['updater'] === 'notice') {
						$result = $this->db->select(
							'tasks_notices',
							array('notice','login'),	
							'`tasks`="'.$f['id'].'" AND `date`="'.$f['updated'].'" AND `private`=0'
						);
						if(is_array($result)) {
							$res = $result[0]['notice'];
							// trim notice
							...
							$res = str_replace("[[/u]]", '</u>', $res);
							$res = preg_replace('~\[\[a (.*)\]\](.*)\[\[/a\]\]~', '<a href="$1" target="_blank">$2</a>', $res);
	...
	function get_response() {
		...
		if(isset($this->settings['form']['reporter'])) {
			$s->add(array('reporter', $this->lang['filter_my_task']), array(0,1));
		}
		...
		foreach($elements as $v) {
			if(isset($settings['form'][$v])) {
				...
				$d['filter_'.$v]['object']['attrib']['handler'] = 'onmousedown="phppublisher.select.submit=true;phppublisher.select.init(this, \''.$this->lang[$v].'\'); return false;"';
