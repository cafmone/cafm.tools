...
var $lang = array();
	...
	function action($action = null) {
		...
		if($this->forwarder === 'insert' && 
			(
				!isset($this->ticket['group']) && 
				!isset($this->ticket['supporter'])
			)
		) {
		...
		## TODO check $this->ticket['group'] is null?
		else if($this->forwarder === 'update' && 
			(
				!isset($this->ticket['group']) && 
				!isset($this->ticket['supporter']) && 
				!isset($this->ticket['reporter']) && 
				!isset($this->ticket['reporter_email'])
			)
		) {
			$this->response->redirect(
				$this->response->get_url(
		...
		$response = $this->mail();
		$t = $response->html->template($this->tpldir.'/ticket.mail.html');
		$t->add('', 'error');
	...
	function __getmail($form) {
		...
		if($form->get_request('to_group') || $form->get_static('to_group')) {
			$result = $this->user->query->select('users2groups', array('login'), array('group', $this->ticket['group']));
			if(is_array($result)) {
				foreach($result as $v) {
					$users[] = $v['login'];
				}
	...
	function get_response( $mode ) {
		...
		if(isset($this->controller->settings['email']['auto'])) {
			...
			if($user['login'] === $this->ticket['supporter']) {
				$this->controller->settings['email']['to_reporter'] = 'true';
			}
			else if($user['login'] === $this->ticket['reporter']) {
		...
		if( $mode === 'update' ) {
			...
			foreach($elements as $v) {
				if(isset($this->ticket[$v])) {
					$option = $this->db->select('ticket_form', array('option'), array('id', $this->ticket[$v]));
					if(is_array($option)) {
						$option = array_shift($option);
						$this->ticket[$v] = $option['option'];
					}
					else {
						$this->ticket[$v] = '';
					}
			...
			$d['to_group'] = '';
			if(isset($this->controller->settings['form']['group']) && isset($this->ticket['supporter'])) {
				$d['to_group'] = array();
				...
				#if(isset($this->ticket['group'])) {
					$d['to_group']['object']['attrib']['value'] = $this->ticket['group'];
				#}
			}
			$d['to_supporter'] = '';
			if(isset($this->controller->settings['form']['supporter']) && isset($this->ticket['supporter'])) {
				...
				$d['to_supporter']['object']['attrib']['name'] = 'to_supporter';
					...
					$d['to_supporter']['object']['attrib']['value'] = $this->ticket['supporter'];
				#}
			}
			...
			if(isset($this->controller->settings['form']['reporter'])) {
				...
				if(isset($this->ticket['reporter'])) {
					$d['to_reporter']['object']['attrib']['value'] = $this->ticket['reporter'];
				}
			}
			...
			if(isset($this->controller->settings['email']['subject'])) {
				...
				$d['subject']['object']['attrib']['value'] = sprintf($this->lang['label_ticket'], $this->id);
			}
			if(isset($this->controller->settings['email']['custom_subject'])) {
			...
			if(is_array($tplvars)) {
				foreach($tplvars as $k => $v) {
					switch($k) {
						case 'id':
							$tplvars[$k] = sprintf($this->lang['label_ticket'], $v);
						break;
						case 'reporter_email':
			...
			$t = $response->html->template($this->controller->profilesdir.'/ticket/templates/ticket.mail.tpl');
			$t->add($tplvars);
			$body = $t->get_string();
			$nid = $response->html->request()->get('notice_id');
			if($nid !== '') {
				$result = $this->db->select('ticket_notices', array('notice'), array('id', $nid));
				if(is_array($result)) {
					$result = array_shift($result);
