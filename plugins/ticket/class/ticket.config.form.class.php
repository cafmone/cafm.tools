...
var $tpldir;
	...
	function __construct( $controller ) {
		...
		$this->lang       = $this->user->translate($this->lang, CLASSDIR.'plugins/ticket/lang/', 'ticket.config.form.ini');
	}
	...
	function action($action = null) {
		...
		$tab = $this->response->html->tabmenu('ticket_elements');
	...
	function element( $element, $hidden = true) {
		$data = '';
		if( $hidden === false ) {
			require_once(CLASSDIR.'plugins/ticket/class/ticket.config.element.class.php');
			$this->action = $element;
			$controller = new ticket_config_element( $this->response, $this->db, $element);
	...
	function group( $hidden = true ) {
		...
		if( $hidden === false ) {
			...
			// EXISTING GROUPS
			$groups = array();
			$result = $this->db->select('ticket_form', array('option', 'id'), array('element', 'group'));
			if(is_array($result)) {
				foreach($result as $v) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($groups as $id => $option) {
					...
					if(in_array($option, $ini)) {
						...
						unset($ini[$rank]);
						$error = $this->db->update('ticket_form', array('rank' => $rank, ), array('id', $id));
					}
					else if(!in_array($option, $ini)) {
						$error = $this->db->delete('ticket_form', array('id', $id));
					}
					if($error !== '') {
						$errors[] = $error;
					}
				...
				foreach($ini as $k => $v) {
					$error = $this->db->insert('ticket_form', array('rank' => $k, 'option' => $v, 'element' => 'group' ));
					if($error !== '') {
						$errors[] = $error;
					}
				}
			...
			$t = $response->html->template($this->tpldir.'ticket.config.form.html');
			$t->add($vars);
			$t->add($response->form);
