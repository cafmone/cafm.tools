...
var $tpldir = '';
	...
	function __construct( $controller ) {
		...
		$this->lang       = $controller->user->translate($this->lang, CLASSDIR.'plugins/ticket/lang/', 'ticket.config.email.ini');
	}
	...
	function action($action = null) {
		...
		switch( $this->action ) {
			...
			case 'update':
				...
				$t = $response->html->template($this->tpldir.'ticket.config.email.html');
				$t->add($vars);
				$t->add($response->form);
			...
			case 'template':
				...
				$t = $response->html->template($this->tpldir.'ticket.config.email.template.html');
				$t->add($vars);
				$t->add($response->form);
				$t->group_elements(array('param_' => 'form'));
				return $t;
	...
	function template( $template ) {
		...
		if(!$form->get_errors() && $response->submit()) {
			...
			$error = $this->file->mkfile($this->controller->profilesdir.'ticket/templates/ticket.mail.tpl', $template, 'w+', true );
			if($error === '') {
	...
	function get_response( $mode ) {
		...
		if( $mode === 'update' ) {
			...
			if(file_exists($this->controller->profilesdir.'/ticket/templates/ticket.mail.tpl')) {
				$d['template']['object']['attrib']['value'] = $this->file->get_contents($this->controller->profilesdir.'/ticket/templates/ticket.mail.tpl');
			}
			$form->add($d);
		}
		$response->form = $form;
