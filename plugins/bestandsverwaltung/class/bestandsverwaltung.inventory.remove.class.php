<?php
/**
 * bestandsverwaltung_inventory_remove
 *
...
class bestandsverwaltung_inventory_remove
{
	...
	function action($action = null) {
		if($this->is_valid) {
			...
			if(!isset($response->msg)) {
				...
				$t = $this->response->html->template($this->tpldir.'/bestandsverwaltung.inventory.remove.html');
				$t->add('Anlage(n) entfernen', 'label');
		...
		if(!isset($response->msg)) {
			...
			if(!$form->get_errors() && $response->submit()) {
				...
				foreach($request as $key => $id) {
					...
					if($error === '') {
						$error = $this->db->delete('bestand', array('id', $id));
						if($error === '') {
							$form->remove($this->controller->identifier_name.'['.$key.']');
							$message[] = sprintf('removed id %s', $id);
						} else {
							$errors[] = $error;
						}
					} else {
