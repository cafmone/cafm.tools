...
phppublisher = {
	...
	modal : {
		...
		init : function(data, id) {
			...
			content.className = 'modal-content panel-default';
			...
			header.className = 'modal-header panel-heading';
	...
	select : {
		...
		init : function(element, label) {
			...
			find.className = 'form-control input-sm text';
			find.style.width = '200px';
			find.style.position = 'absolute';
			find.style.top = '5px';
			find.style.right = '30px';
			find.id = 'searchbox';
			find.tabIndex = '0';
			//find.title = 'Example';
			find.setAttribute('placeholder', 'Search');
			//find.setAttribute('data-toggle', 'poover');
			//find.setAttribute('data-placement', 'bottom');
			//find.setAttribute('data-trigger', 'hover');
			//find.setAttribute('data-content', '<center>Lorem.+Do matches<br> \'Lorem ipsum do\' in \'Lorem ipsum dolor sit amet\'</center>');
			...
			//setTimeout(function() {
			//	$('#searchbox').popover({placement: 'bottom', html: true});
			//}, 10);
			...
			phppublisher.modal.label = label;
			this.modalid = phppublisher.modal.init(content);

			//this.fake = phppublisher.modal.init(wait, 'fakemodal');
			//$('#'+this.fake).modal({ backdrop: true, show: true, keyboard: true});
...
/*
			drawLoop = function(element, box, i) {
				//console.log(i);
				opt = document.createElement("a");
				opt.className = 'btn btn-default btn-sm btn-block';
				opt.id = 'opt'+i;
				if(element.selectedIndex != 0 && i == element.selectedIndex) {
					opt.className = 'btn btn-primary btn-sm btn-block';
				}
				opt.style.textAlign = 'left';
				opt.innerHTML = element.options[i].text;
				...
				if(i < element.length) {
					repeater = requestAnimationFrame(function() {
						drawLoop(element, box, i);
					});
				}
			}
			drawLoop(this.element, this.box, 0);
*/
